...
2. ⬜ Make the project-specific tables (Approved Manufacturers, Owner Supply) data-driven rather than HTML templates.
3. ⬜ Excel device-schedule annex (proper spreadsheet, not the Word room list) — reuse the Feature Matrix Excel export pattern.
4. ⬜ CSI output polish — cover page, in-Word table of contents (the on-screen navigator + section numbering are done).
5. ⬜ Fill gaps — Lighting and Radio category/general/execution text; revisit Security (deferred).
6. ⬜ Product compliance checking (bigger) — tag products with capabilities, flag placements that miss a requirement; promote spec_text lines to atomic checkable items.
