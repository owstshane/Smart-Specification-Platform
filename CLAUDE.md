...
### Global Library
- `global_equipment` — device_type_id, description, category_code, subcategory_code, system_group, spec_ref, spec_text (features/functions override), network_role, avip_role, audio_role, lan_ports, poe_ports, poe_type, poe_budget_watts, power_types (array), power_watts, heat_btu, ups_protected, speaker_channels, spk_watts, budget_price, currency
...
- `equipment_instances` — project_id, device_id (UNIQUE per project, DB-enforced), device_type_id, description, room_id, rack_zone_id, quantity, installation_type, notes, **change_status, change_cr_id, change_cr_ref**
...
- ✅ Branded Word formatting — central `SPEC_STYLE` config (fonts, colours, heading sizes, A4 page, margins, `logoUrl`, `swoopUrl`) drives `_wrapWordDoc`: A4 + 2cm margins via `@page`, content in a `WordSection1`, repeating header (brand/logo + project) and footer (project + Page field), via the standard Word-HTML mso-element technique.
- ✅ CSI cover page redesigned to house spec — `_specCoverHtml(today,revLabel)`: official SMART **swoop** (embedded as inline SVG in `SPEC_SWOOP_SVG`; `SPEC_STYLE.swoopUrl` overrides with a hosted raster), then all-caps lines: PROJECT NAME / SMART TECHNOLOGY ADVISERS / (gap) / TECHNOLOGY SPECIFICATION / (gap) / REVISION <num from latest project_revisions, e.g. R1.0> / DATE.
- ✅ House-style rules (Jun 2026): whole spec is **Arial** (`SPEC_STYLE.font` + `.spec-preview-doc`). Official SMART **wordmark** (`SPEC_LOGO_SVG`, inline SVG) repeats **top-right in the page header** on body pages, **suppressed on the cover** via `mso-title-page` + empty first-page header/footer. **Footer** shows the page number (PAGE field), also suppressed on the cover. **Every `h1` starts a new page** (`h1{page-break-before:always}` in the Word CSS); contents page no longer has a trailing break.
- ⏳ **FORMATTING TO REVISIT (parked):** (a) "header on all pages except first AND last" — only the first (cover) is suppressed; HTML-to-Word can't skip just the last page (needs true `.docx`). (b) The header/footer + page-numbers + first-page suppression + H1 page-breaks all use the standard Word-HTML `mso-element` technique and have NOT been confirmed in desktop Word yet — verify there and adjust. (c) `SPEC_STYLE.logoUrl` can point the header at a hosted raster wordmark if the inline SVG doesn't render in Word. (d) Eventual full-fidelity path: generate a true `.docx` (CDN docx library or a Word template) — the content engine is output-agnostic so it reuses everything (sections, numbering, overrides, annexes, cover, contents).

**Other backlog:**
7. ⬜ CO Document generation — formal Change Order PDF/Word from a CR's cost schedule.

**Where to resume:** spec generation lives in the JS as `_assembleCsiSpec()` (builds the HTML body, applies overrides, registers `.spec-block` editable blocks + custom sections via `renderCustoms()`, then numbers the main body with `_numberSpecHtml()`; shared by export + preview), `downloadSpecFromPreview()` (downloads Word from the cached preview; `generateCsiSpec()` is dead code, no call sites), `previewCsiSpec(keepScroll)` (renders inline into the Specification page; `loadSpecification` calls it; also runs `_buildSpecToc()`), `generateRoomSpec()`. Inline editing: `_decorateSpecBlocks` / `specEditBlock` / `specSaveBlock` / `specRevertBlock`, with `_specEditMode` = lines | quill | html. Custom sections (Phase B): `addCustomSection(part)` / `specDeleteCustom`. Contents sidebar: `_buildSpecToc` / `_specTocGo`. Per-zone engineering loads come from shared `computeZoneLoads()`. Schedule HTML is shared `_equipScheduleHtml()` / `_groupedEquipList()`. Text precedence: project override > device type > sub-category > category. Taxonomy: System Group > Category > Sub-category > Device Type. For SaaS later: tenancy = add `org_id` to the global tables, NOT per-project copies (decided).
...
- **Full code review + trust-layer fixes (Jul 2026):** Five-agent review of the whole file (correctness, security, data integrity, intent/dead code, maintainability/perf), then the top findings fixed in 9 commits. Change control: `stampChange(payload,status,existing)` now keeps 'added' provenance when a row added under the active CR is edited, and removing a same-CR addition hard-deletes it, so rejected CRs can no longer leave rejected equipment in the baseline; `rejectCR`/`deleteCR` now call transactional Postgres functions `reject_cr`/`delete_cr` (security invoker, one all-or-nothing call; SQL in docs/draft-migration-cr-rpcs.sql); deleting an approved CR keeps its changes and badges but unlinks change_cr_id; `closeCR`/`deleteCR` clear the active CR, and `validateActiveCR()` re-checks a restored active CR on project open. Equipment: new `_peFromGlobal()` is the single payload builder for all pull paths (heat/UPS/PoE budget/power/subcategory_code/budget_price all survive; template apply now stamps + logs); soft-removed instances excluded from Zone Summary (inside `computeZoneLoads`), Feature/Equipment matrices and Excel exports. Device IDs are unique per project: DB unique index on (project_id, device_id), 206 legacy duplicates renumbered (first kept its ID, copies suffixed -2, -3...), generators use max suffix + a project-wide ID set. Security: `escJs()` fixes the apostrophe-in-onclick bug (46 sites; escHtml alone breaks because the parser decodes entities before JS runs); `safeColour()` hex whitelist closes a stored XSS via rack zone colours; `sanitizeSpecHtml` fails closed if DOMPurify is missing; error messages escaped; RLS enabled on the 5 bak_glib_* backup tables (were open to the anon key). Auth: multi-tab fix, `refreshTokenIfNeeded(force)` adopts a newer localStorage token, 401 retry forces refresh, dead session shows the login overlay (`forceRelogin`). DB also gained: `global_equipment.power_watts` + `poe_budget_watts` (modal saves were failing on the missing column), unique CR numbers per project/type, one contract award per project. Word download path is `downloadSpecFromPreview()`; `generateCsiSpec()` is dead. Known remaining from the review: placed-equipment import round-trip broken (never sets project_equipment_id), import is N+1 (row-by-row inserts), spec-block editor allows two open panels to cross-contaminate, custom-section sort_order uses Date.now()%1e6, Open CR count differs between Project Info and Change Control, phase badges have no CSS, ~8k dead code (legacy ie* import cluster). Advisor notes: RLS policies are all authenticated-full-access (consider admin-only globals), leaked password protection off in Auth settings. Global library content gaps: only 29/127 device types have a sub-category, none have heat_btu.
